...
def _make_candidate(
    ...
    stock = StockInfo(
        symbol=symbol, name=name, code=symbol, percent=percent, current=15.0, value=5000, rank_change=100, rank=rank
    )
    kline = KlineSummary(
        trend="test",
    ...
    return Candidate(stock=stock, category=category, score=30, reason="test", kline=kline)


def _kdims(c):
    """测试助手：返回候选的 dimensions（窄化 c.kline 可空类型，消除 Pyright 存量噪音）。"""
    assert c.kline is not None, "测试候选必须带 kline"
...
class TestAccumulateFinalScore:
    def test_all_zeros(self):
        stock = StockInfo(
            symbol="300999", name="测试", code="300999", percent=0.0, current=10.0, value=1000, rank_change=0, rank=50
        )
    ...
    def test_basic_sum(self):
        stock = StockInfo(
            symbol="300999", name="测试", code="300999", percent=5.0, current=15.0, value=5000, rank_change=100, rank=30
        )
        c = Candidate(stock=stock, category="new_face", score=10, reason="test", kline=None)
...
class TestApplyListMomentumBonus:
    ...
    def test_fatigue_new_face_skips_price(self, mock_traj):
        c = _make_candidate(accumulated_pct=3.0, volume_ratio=1.5, percent=2.0, category="new_face")
        _apply_list_momentum_bonus(c, list_streaks={"300999": 1}, cross_days=2)
        assert c.list_momentum_bonus == 5
    ...
    def test_accelerating_writes_separate_fatigue_accelerate_key(self, mock_traj):
        ...
        _apply_list_momentum_bonus(c, list_streaks={"300999": 1}, cross_days=2)
        assert c.list_momentum_bonus == 6
        assert _kdims(c).get("fatigue_accelerate") == 6
        assert _kdims(c).get("fatigue") is None, (
            f"fatigue 键应保持未写入（仅疲劳罚分写负值）, got {_kdims(c).get('fatigue')}"
        )
    ...
    def test_fatigue_rebound_skips_price_signal(self, mock_traj):
        ...
        c.category = "rebound"
        _apply_list_momentum_bonus(c, list_streaks={"300999": 1}, cross_days=2)
        assert c.list_momentum_bonus == 5, f"rebound 应豁免价格信号 → streak=3 榜单动能 5, got {c.list_momentum_bonus}"
        assert _kdims(c).get("fatigue") is None
...
class TestRecordDimensions:
    def test_records_sector_and_vol(self):
        ...
        assert _kdims(c)["sector_bonus"] == 4
        assert _kdims(c)["live_vol_bonus"] == 5
        assert _kdims(c)["intraday_score"] == 0.8
        assert _kdims(c)["opening_score"] == 0.6

    def test_market_env_strong(self):
        c = _make_candidate()
        _record_dimensions(c, 1.0, {})
        assert _kdims(c)["market_env_bonus"] == 2
    ...
    def test_market_env_neutral(self):
        ...
        assert "market_env_bonus" not in _kdims(c)
...
class TestMainForceDistributionFlicker:
    ...
    def _make_dist_candidate(self, intraday_score=None, today_pct=0.0, accumulated=15.0, opening_score=5.0):
        c = _make_candidate(accumulated_pct=accumulated, percent=today_pct)
        _kdims(c)["opening_score"] = opening_score
        if intraday_score is not None:
            c.intraday_score = intraday_score
            _kdims(c)["intraday_score"] = intraday_score
        return c
    ...
    def test_intraday_near_zero_no_flicker(self):
        ...
        for intra in [-0.5, -0.3, 0.0, 0.3, 0.5]:
            ...
            assert not _detect_main_force_distribution(c, _kdims(c)), (
    ...
    def test_intraday_clearly_weak_triggers(self):
        ...
        assert _detect_main_force_distribution(c, _kdims(c))
    ...
    def test_today_pct_near_threshold_no_flicker(self):
        ...
        assert not _detect_main_force_distribution(c, _kdims(c)), "today_pct=0.8% 在过渡区（0.5~1.0），不应触发滞涨"
...
class TestRiskFlagTightening:
    ...
    def test_today_breakout_ma_support_no_trend_breakage(self):
        ...
        assert _detect_trend_breakage(_kdims(c2)), "今日仍在 MA5 下方（BROKEN）应判趋势破位（真破位止损信号）"

    def test_distribution_rule2_needs_overheated_turnover(self):
        """主力出货 Rule 2：累计≥15% + 超买旗，但换手仅'活跃'(>0 非过热) 不应触发。"""
        c = _make_candidate(category="momentum", accumulated_pct=18.0, volume_ratio=1.5)
        c.turnover_bonus = 3  # 换手 5~10%：活跃但 < 20%（非过热）
        _kdims(c)["mo_overbought_flag"] = True  # 已收紧的极端超买旗
        assert not _detect_main_force_distribution(c, _kdims(c)), (
    ...
    def test_distribution_rule2_triggers_on_overheated_turnover(self):
        ...
        _kdims(c)["mo_overbought_flag"] = True
        assert _detect_main_force_distribution(c, _kdims(c)), "Rule 2 过热换手 + 极端超买应判出货"

    def test_low_turnover_high_accum_not_distribution(self):
        """累计很高但换手低迷（无放量），不应被'高位高换手'规则误判。"""
        c = _make_candidate(category="momentum", accumulated_pct=20.0, volume_ratio=1.2)
        c.turnover_bonus = 0  # 换手 ≤5%：低迷
        _kdims(c)["mo_overbought_flag"] = True
        assert not _detect_main_force_distribution(c, _kdims(c)), "高累计+低迷换手不应判主力出货（无放量派发特征）"
...
class TestDistributionRule5BackrowIntradayWeak:
    ...
    def _rule5_candidate(intraday, v_st_rank=V_ST_RANK_LOW, accumulated=10.0):
        ...
        c = _make_candidate(category="short_term", rank=99, accumulated_pct=accumulated, percent=3.5)
        _kdims(c)["v_st_rank"] = v_st_rank
        c.intraday_score = intraday
        _kdims(c)["intraday_score"] = intraday
        return c
    ...
    def test_backrow_intraday_weak_triggers(self):
        ...
        assert _detect_main_force_distribution(c, _kdims(c)), "后排+盘中弱应触发 Rule 5（300317 型画像）"
    ...
    def test_backrow_intraday_borderline_no_trigger(self):
        ...
        assert not _detect_main_force_distribution(c, _kdims(c)), "intraday=-1.4 未达 -1.5 阈值不应触发"

    def test_backrow_intraday_neutral_no_trigger(self):
        """后排但盘中中性（intraday=0）不触发：分时走弱是必要条件。"""
        c = self._rule5_candidate(intraday=0.0)
        assert not _detect_main_force_distribution(c, _kdims(c)), "盘中中性不应触发 Rule 5"
    ...
    def test_front_rank_intraday_weak_no_trigger(self):
        ...
        for bonus in (V_ST_RANK_TOP30, V_ST_RANK_TOP10):
            ...
            assert not _detect_main_force_distribution(c, _kdims(c)), f"前排(v_st_rank={bonus})不应触发 Rule 5"
...
class TestWtsFailureHardFilter:
    ...
    def _wts_candidate(intraday, wts=8):
        ...
        _kdims(c)["v_st_weak"] = wts  # 弱转强直通维度
        c.intraday_score = intraday
        _kdims(c)["intraday_score"] = intraday
        return c
...
class TestApplyFundFlowBonus:
    ...
    def _flow(c, main_pct, main_net, super_net):
        return {c.stock.symbol: {"fund_flow": {"main_pct": main_pct, "main_net": main_net, "super_net": super_net}}}
    ...
    def test_strong_inflow(self):
        ...
        assert _kdims(c).get("fund_flow_main_pct") == 8.5
        assert _kdims(c).get("fund_flow_main_net") == 123456789.0
    ...
    def test_garbage_flow_values_no_crash(self):
        ...
        c = _make_candidate()
        _apply_fund_flow_bonus(
            c, {c.stock.symbol: {"fund_flow": {"main_pct": "abc", "main_net": "NaN", "super_net": None}}}
        )
        assert c.fund_flow_bonus == 0
        assert _kdims(c).get("fund_flow_main_pct") == 0.0
    ...
    def test_inf_flow_coerced(self):
        ...
        _apply_fund_flow_bonus(
            ...
            {
                c.stock.symbol: {
                    "fund_flow": {"main_pct": float("inf"), "main_net": float("-inf"), "super_net": float("nan")}
                }
            },
        )
        assert c.fund_flow_bonus == 0
        v = _kdims(c).get("fund_flow_main_pct")
        assert v == 0.0  # inf/NaN 已兑底为 0.0（0.0 必然有限，替代原 isfinite 断言）
        assert _kdims(c).get("fund_flow_main_net") == 0.0
        assert _kdims(c).get("fund_flow_super_net") == 0.0


class TestApplyZtBonus:
    @staticmethod
    def _zt(c, lianban, zhaban, industry="软件"):
        return {c.stock.symbol: {"zt": {"lianban": lianban, "zhaban": zhaban, "industry": industry}}}

    def test_3board_momentum(self):
        c = _make_candidate(category="momentum")
        _apply_zt_bonus(c, self._zt(c, 3, 0))
        assert c.zt_lianban_bonus == 8
        assert _kdims(c).get("zt_lianban") == 3
    ...
    def test_garbage_zt_values_no_crash(self):
        ...
        c = _make_candidate(category="momentum")
        _apply_zt_bonus(c, {c.stock.symbol: {"zt": {"lianban": "abc", "zhaban": "x", "industry": None}}})
        assert c.zt_lianban_bonus == 0
        assert _kdims(c).get("zt_lianban") == 0
        assert _kdims(c).get("zt_zhaban") == 0
...
class TestTurnedRedGapTodayBarGuard:
    """2026-09-04 审查修复回归：翻绿+高开回落只认 date==today 的今日 bar。

    此前 kl[-1] 无日期校验：K 线补拉失败（序列末位是昨日 bar）时，会拿昨日
    形态打"当日翻绿+高开回落"硬过滤（RISK_FLAGS_HARD_FILTER 成员）。
    """

    YESTERDAY = "2026-09-03"
    TODAY = "2026-09-04"

    def _klines_ending_yesterday(self, symbol: str) -> dict:
        # 末位（昨日）bar 恰好是"翻绿+高开回落"形态：
        # prev_close = 10.5/1.02 ≈ 10.29 < open=11.0（高开），close=10.5 < open=11.0（翻绿）
        return {
            symbol: [
                {"date": "2026-09-02", "open": 10.0, "close": 10.2, "high": 10.3, "percent": 2.0},
                {"date": self.YESTERDAY, "open": 11.0, "close": 10.5, "high": 11.5, "percent": 2.0},
            ]
        }

    def test_stale_bar_no_false_flag(self):
        from scanner.candidates import candidate_excluded_by_risk
    ...
    def test_today_bar_flagged(self):
        c = _make_candidate(symbol="SZ300999")
