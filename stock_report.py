...
def main():
    ...
    if isinstance(current_price, (int, float)):
        pct_str = f"  ({green(f'+{live_pct:.2f}%') if live_pct and live_pct > 0 else red(f'{live_pct:.2f}%') if live_pct else ''})" if live_pct else ""
    ...
    if kline_data and len(closes) >= 5:
        ...
        if len(closes) >= 20:
            ...
            print(f"  MA20 = {ma20:.2f}  {'(↑ 趋势向上)' if ma20 > sum(closes[-21:-1]) / 20 else '(↓ 趋势向下)'}")
