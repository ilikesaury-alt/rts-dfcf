...
# sys.stdout 静态类型是 TextIO（无 reconfigure 属性）；运行时 TextIOWrapper 才有（同 unified_scanner）
_reconfigure = getattr(sys.stdout, "reconfigure", None)
if callable(_reconfigure):
    _reconfigure(encoding="utf-8")

parser = argparse.ArgumentParser(description="榜单可观测性：雪球飙升榜/热搜榜成分与排名分布时间序列")
parser.add_argument(
    "--date", default=None, help="目标日期 (YYYY-MM-DD)，默认今日（盘中有则看盘中，无则看最近有数据的一天）"
)
parser.add_argument("--days", type=int, default=7, help="分日汇总回看天数（默认7）")
parser.add_argument("--source", default="biaosheng", choices=["biaosheng", "hot"], help="数据源，默认飙升榜")
...
has_table = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='leaderboard_log'").fetchone()
if not has_table:
...
def detect_anomalies(conn, source, target):
    ...
    if med is not None and pmed is not None and abs(med - pmed) >= 1.0:
        signs.append(
            f"{target} 榜单中位涨幅 {med:.2f}% vs 昨日 {pdate} {pmed:.2f}%，突变≥1pp，检查是否上游排序口径变更"
        )
    if ov is not None and ov < 0.3:
        signs.append(f"{target} 最近一轮重叠率仅 {ov:.0%}，榜单成员剧烈抖动（分页/排序不稳）")
    if total and ptot and abs(total - ptot) / ptot >= 0.3:
        signs.append(f"{target} 榜单条数 {total} vs 昨日 {ptot}，变动≥30%（样本过滤条件疑似变更）")
    return signs
...
def find_date():
    ...
    row = conn.execute("SELECT date FROM leaderboard_log WHERE source=? AND date=? LIMIT 1", (source, today)).fetchone()
    if row:
        return today
    # 盘中还没数据 → 回退最近有数据的一天
    row = conn.execute("SELECT MAX(date) FROM leaderboard_log WHERE source=?", (source,)).fetchone()
...
if rows:
    ...
    print(f"{'时间':<8}{'条数':>5}{'GEM':>5}{'涨':>4}{'跌':>4}{'中位%':>7}{'前10均%':>8}{'重叠率':>7}{'中位换位':>8}")
    step = max(1, len(rows) // 12)
    for i, r in enumerate(rows):
        if i % step != 0 and i != len(rows) - 1:
            continue
        t, total, gem, up, down, flat, med, _mean, top10, mx, ov, rc = r
        print(f"{t:<8}{total:>5}{gem:>5}{up:>4}{down:>4}{fmt(med):>7}{fmt(top10):>8}{fmt(ov, 2):>7}{fmt(rc):>8}")
    # 异常信号检测：与昨日同源对比
    print("\n■ 口径漂移检测")
    ...
    print(f"\n{target} 无 {source} 数据（盘中扫描时间序列逐轮落库，需启动扫描器并交易时段运行）")

# ---------- 分日汇总 ----------
print(f"\n■ 近 {args.days} 日逐日汇总（{src_label}）\n")
print(f"{'日期':<12}{'扫描轮数':>7}{'均条数':>7}{'均中位%':>8}{'涨/跌':>8}{'前10均%':>8}{'均重叠率':>8}")
...
for d, scans, tot, med, up, down, top10, ov in day_rows:
    print(
        f"{d:<12}{scans:>7}{tot:>7}{fmt(med):>8}{f'{int(up or 0)}/{int(down or 0)}':>8}{fmt(top10):>8}{fmt(ov, 3):>8}"
    )
