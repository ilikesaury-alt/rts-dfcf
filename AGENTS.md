...
scanner/
  ...
  pick.py                 # 今日选股建议（从综合排序候选中挑 2 只，数据驱动）
...
- **次日大涨归因（2026-08-10，`scanner/nextday_attribution.py`）**：用户偏好「次日大涨」票（next_day ≥7%），与系统 cum_3d 决策口径是**两个不同目标**（cum_3d 好的 8-12% 档在 next_day 口径反而 -1.32% 次日回吐）。本模块是**独立的 next_day 校准仪表盘**：分策略 hit 率/rank-IC、推荐时刻涨幅带矩阵、score 分桶、落库维度归因、二元因子条件 hit 率。**当前结论**（去重 1006 条，hit 10.2%）：① 涨幅带甜蜜区在 <1% 与 1-2%（hit 11.7%/13.2%，低吸潜伏）、4-6%/6-8%（11.8%）；**8-10% 是陷阱**（7.5% 且平均 -1.42%）——与 cum_3d 口径「8-12% 最好」相反；② score <30 桶 hit 率最高（16.7%）、70-90 最差（7.4%），**低分反指**再次验证；③ rebound 仍是最强类别（hit 32%、IC+0.26）；④ short_term 超买 hit 仅 5%（非超买 10.5%），弱转强 hit 11.8%。**用法**：`python -m scanner.nextday_attribution [--days N] [--threshold 7] [--csv prefix]`，随数据积累持续迭代观察；任何「次日大涨」目标调参都必须先跑本模块，再跑 `scanner.backtest --metric next_day_pct` 复核，勿与 cum_3d 口径混淆。不进入实时扫描路径。
- **次日大涨候选独立区（2026-08-10，`display.py`）**：方案 A（display-only 观察窗口，用户选定）。综合排序末尾新增「◆ 次日大涨候选」区，把**推荐时刻涨幅在甜蜜带**（<2% 低吸潜伏 或 4~8% 中段启动，`NEXTDAY_SPIKE_SWEET_LOW/MID_*`）且**非 short_term 超买**（死亡信号）的票独立列出，按 `NEXTDAY_CAT_PRIORITY`（rebound>short_term>momentum>kNF>new_face）排序。**只筛形展示**：不改 score / 排序键 / 不落库，主表、回马枪、选股建议均不受影响；无甜蜜带票时不渲染空区。筛选用扫描快照 percent（与 nextday_attribution 口径同源），展示用 live_percent（实时）。后续样本足够、归因稳定后再评估是否并入评分（方案 B）。
...
- **今日选股建议**（2026-08-10，`scanner/pick.py`，接入 `display.py:display_priority` 末尾结论区）：从综合排序候选中挑 2 只买入。**设计依据**：跨类别 score 排序是反指——历史回测"按 score 取前2" cum_3d -3.1% vs 随机 -2.4%（score 是热度放大器，追高次日回撤）；而类别本身有强区分度（rebound 全期 +7.6%/胜率76%、momentum 全期 +2.3% 但近30天弱市 -3.4%、short_term +0.4%）。规则：① 排除负期望类别（new_face/kNF/pullback/comeback）② 排除硬风险标签（主力出货/趋势破位）③ 排除劣后档（主力净流出≤-5%）④ 弱市（`market_env_bonus<0`）屏蔽 momentum ⑤ 类别优先级 rebound > short_term > momentum ⑥ 板块去重（同板块只留1只，不足则放宽）。仅当 `_session_state.today_pool` 非空（真实扫描环境）时渲染，避免干扰 display 测试的行数契约；picks 行/reasons 均不含 symbol 代码（display 测试以 symbol 行数验证主表）。
