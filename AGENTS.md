...
- **推荐后快速反转移出（2026-08-13，`database.py:mark_reversed_recommendations` + `unified_scanner.py`）**：今日已推荐（榜上主类别，不含回马枪）但当前不在候选池的票，同时满足「实时涨幅已转负」与「较推荐时刻回落 ≥ `REVERSAL_DROP_THRESHOLD=3.0`（百分点）」→ 标 `excluded=1` 移出综合排序展示（保留落库记录），`[反转移出]` 告警。**阈值来自历史数据校准**：全量推荐「推荐时刻涨幅−当日收盘涨幅」漂移 p90=2.91（p50 0.11 / p75 1.36）→ 回落≥3 为前 10% 异常漂移；历史回放中位每天命中 1 条，仅崩盘日爆量（2026-06-08 50 条）。**两次教训**：① 仅按回落阈值会把强势股正常回吐（+11%→+7%）成批误杀（曾过滤掉一半）——必须同时要求已转负；② 阈值不可为凑单票（行云科技 +3.86%→-3.12% 回落 6.98，任何 ≥2 阈值都能捕获）而设。回马枪为掉榜跟踪池（推荐时刻=企稳点、尾盘转负常态），不参与；当前候选不碰（orchestrator `passed_syms` 每轮置回 0）；行情缺失 fail-open。仅展示层生效，backtest/nextday_attribution 不过滤 excluded。
