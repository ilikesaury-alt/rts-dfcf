...
- **次日大涨候选独立区（2026-08-10，`display.py`）**：方案 A（display-only 观察窗口，用户选定）。综合排序末尾新增「◆ 次日大涨候选」区，把**推荐时刻涨幅在甜蜜带**（<2% 低吸潜伏 或 4~8% 中段启动，`NEXTDAY_SPIKE_SWEET_LOW/MID_*`）且**非 short_term 超买**（死亡信号）的票独立列出，按 **辨识度(↻) 优先**（2026-08-10，hit 14.5% vs 非辨识度 8.2% 最强单因子）→ `NEXTDAY_CAT_PRIORITY`（rebound>short_term>momentum>kNF>new_face）→ 评分降序。**只筛形展示**：不改 score / 排序键 / 不落库，主表、回马枪、选股建议均不受影响；无甜蜜带票时不渲染空区。筛选用扫描快照 percent（与 nextday_attribution 口径同源），展示用 live_percent（实时）；辨识度复用 `c.prominence_labels` / `entry["_prominent"]`（`_entry_prominent`），↻ 标记由共享渲染器已输出、无需新增数据源。后续样本足够、归因稳定后再评估是否并入评分（方案 B）。
...
- 综合排序档位（2026-08-06 引入，`display.py:display_priority`; **2026-08-11 去掉资金流排序**）: 排序键 `(档位, CAT_DISPLAY_PRIORITY, 分数键)`，档位主键**跨类别全局生效**。档0置前 = 辨识度(↻)；档1 = 其余。**资金流不再参与档位/劣后过滤**（原档0含主力净流入≥5%置前、档2净流出≤-5%劣后覆盖辨识度——均移除；净流出票回到正常排序展示，仅保留图标与「资金流出」标签）。档内次键仍为类别优先级→评分（kNF 升序）。档位只改排序，不改评分列/不落库/不影响策略桶与回测。辨识度候选用 `c.prominence_labels`、掉榜行用 `get_prominence_map`——掉榜行 DB score 不含这些字段，展示层统一分档避免同表两套口径。
