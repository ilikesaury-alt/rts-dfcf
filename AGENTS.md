...
其中**只有 `v1 池选` 有序**：**新票优先 → 过热劣后 → 类别优先级 → 榜单排名升序 → 资金流降序 → 形态加分**
（实现在 `scanner/view/assemble.py::build_scan_view`；终端标题只列其中 4 项：
`◆ v1 池选 — 新票优先·过热劣后·类别优先·资金流降序`，**第 4 键「榜单排名升序」未入标题** ——
它在榜内票之间真实生效，是否补进标题留给用户定夺长度，见 `scanner/view/render.py` 的注释）。「新票」= 本轮新进入今日推荐池，
