...
- Cross-validation (`validator.py`): each candidate must pass ≥2 of its independent dimensions (pos_dims ≥ 2)，超买判定透传扫描锚定日 `today_str`。
  ...
  - **momentum**: MA5>10>20 alignment (EMA, penalty -5 if broken), no RSI divergence, volume uniformity (5-day window). pos_dims ≥ 2 required.「首次启动」子模式 MA 要求放宽至 score>=0（非空头），原 >= MA_BULL_2_TIER_SCORE（部分多头）在低位启动阶段过于严格。
  ...
  - **comeback（回马枪，`comeback.py`）**: 补掉榜盲区。两变体均 `category="comeback"`（`Candidate.off_list=True` + `comeback_variant`）：
    ...
    - **回踩**: 近 `COMEBACK_REENTRY_DAYS=5` 交易日推荐（排除今日）且不在榜，硬过滤（今日 ±5% / 累计 ±10% / 主力净占比 ≤-5% fail-open），5 维买点信号（均线支撑/缩量/RSI合理/BOLL中轨/MACD未死叉）≥3 才入选；评分 `BASE=40 + 15×信号数`。
