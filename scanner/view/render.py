...
def render_terminal(view: ScanView) -> None:
    ...
    # ── v1 池选 ──
    # 标题 = 实际排序键的**全链**（2026-09-22 用户决策：删 tier 后整条链可入题）。
    # 实现在 assemble.build_scan_view：
    #   sort(key=(is_new, 类别优先级, 榜单排名, -资金流, -形态加分))
    #   第1键 新票优先            ← 本轮新进池（new_symbols 差集）。无新票时该键恒等
    #   第2键 类别展示优先级      ← 「kNF→MOM/NEW→ST」的分组
    #   第3键 榜单排名升序        ← **在生产路径是活的**（rank_map 只覆盖当轮在榜票，
    #                              2026-09-21 17:28 那轮正是它把 rank17 的义翘神州排在
    #                              rank43 的威尔高之前）
    #   第4键 资金流降序          ← 无榜内排名的行靠它定序（组内主力）
    #   第5键 形态加分（低吸/突破标签）← 稀有 tie-breaker，多数行恒 0，**不入标题**
    # ⚠ 沿革：2026-09-16~21 的键里还有第 2 键 tier（composite_tier 过热劣后 / composite
    #   分档），2026-09-22 按用户决策删除 —— 它与类别键高度重合且 composite 部分无法向
    #   用户解释。**副作用**：accum≥50% 过热票不再被排序劣后，只靠行内 ⚠ 标记提示；
    #   需恢复时从 git 历史取回 `composite_tier` 调用（assemble.py）。
    # 标题纪律：标题四词与第 1~4 键一一对应，**改键必须同批改标题**（09-16 换键漏改标题
    # 的教训——标题是用户判断「排序对不对」的唯一线索）。
    print(f"  {ANSI['BOLD']}◆ v1 池选 — 新票优先·类别优先·排名升序·资金流降序{ANSI['RESET']}")
