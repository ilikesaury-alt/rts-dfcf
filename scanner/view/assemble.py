...
def build_scan_view(
    ...
    try:
        ...
        for e in main_recs:
            ...
            # 2026-09-16 的真实结论是「它不再**直接**充当排序键」，**不是**「不再影响排序」：
            # 它经 composite_tier 折算成档位后仍是**排序第一主键**（见下两行）。原注释写作
            # 「依用户决策 2026-09-16 不再作为排序主键」，与紧邻的 tier 行自相矛盾，已更正。
            cs = composite_score(e, conn, accum_map=accum_map)
            # 档位 = 过热硬门（accum≥50% → 3）**优先**；否则按 composite_score 阈值
            # 6.0/4.0/2.0 分 0..3 档。只有前半段是实时安全阀，后半段是回测结论固化成 4 档
            # 粗刻度（cat_base 派生自类别 hit 先验、rank/fund/dip 三项校准于 1949 样本）。
