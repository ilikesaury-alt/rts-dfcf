...
queries / dal 都要用交易日回溯（n_trading_days_ago），放这里避免两个模块
...
def n_trading_days_ago(n: int, as_of: str | None = None) -> str:
    ...
    while trading_days < n:
        ...
        iters += 1
        if iters > max_iter:
            logger.warning("n_trading_days_ago(%d): max_iter=%d 触发, "
                           "回溯仅到达 %s (期望 ~%d 个交易日前), "
