...
from scanner.config import DISPLAY_MAX_TODAY_PCT, now_beijing
...
def market_gate(conn: sqlite3.Connection) -> tuple[bool, str]:
    ...
    try:
        rows = conn.execute(
            "SELECT date, index_pct FROM market_index_log ORDER BY date"
        ).fetchall()
...
def build_decision_picks(conn: sqlite3.Connection, today: str | None = None) -> dict[str, Any]:
    """构建今日决策层：市场门 → 类别先验内取每类头部 → 全局配额截断。

    返回 {"allowed": bool, "gate_reason": str, "picks": [dict], "ts": str}。
    ...
    for sym, name, cat, score, pct in rows:
        if cat in by_cat:
            by_cat[cat].append(
                {"symbol": sym, "name": name or "", "category": cat,
                 "score": score or 0, "percent": pct}
            )
    picks: list[dict] = []
...
def save_decision_picks(conn: sqlite3.Connection, result: dict[str, Any],
    ...
    ts = now_beijing().strftime("%H:%M:%S")
    rows = [(
        rec_date, "__gate__", "", "__gate__", None, None,
        result.get("gate_reason", ""), ts,
    )]
    for p in result.get("picks", []):
        rows.append((
            rec_date, p["symbol"], p.get("name", ""), p["category"],
            p.get("score"), p.get("percent"), "决策层入选", ts,
        ))
    try:
        conn.executemany(
...
def decision_lines(conn: sqlite3.Connection, today: str | None = None) -> list[str]:
    ...
    lines = [f"◆ 今日决策层（≤{DECISION_MAX_PICKS} 只 · 大盘门+类别先验+配额三道门）"]
    ...
    for i, p in enumerate(result["picks"], 1):
        ...
        lines.append(
            f"  {i}. {p['symbol']} {p['name']} [{p['category']}] "
            f"分:{p['score']:.0f} 现价{pct_s}"
        )
    return lines
