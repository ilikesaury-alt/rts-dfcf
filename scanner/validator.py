...
def validate_short_term(stock, kline_summary, closes: list[float],
    ...
    if overbought:
        ...
        # 复用已计算的 pos_dims/non_sector_pos，减去 wts_bonus 维度（超买时失效）。
        wts_dim = 1 if wts_bonus > 0 else 0
        pos_dims_no_wts = pos_dims - wts_dim
        non_sector_pos_no_wts = non_sector_pos - wts_dim
