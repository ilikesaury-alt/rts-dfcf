...
# 候选池的票，同时满足「实时涨幅已转负」与「较推荐时刻回落 ≥ 此阈值（百分点）」才视为推荐
# 失败，标 excluded=1 移出综合排序展示（保留落库记录）。
# 阈值来源（2026-08-13 历史数据校准，非单票凑参）：全量历史推荐「推荐时刻涨幅−当日收盘涨幅」
# 漂移分布 p90 = 2.91 → 回落 ≥3 点属于前 10% 的异常漂移，即"超出正常回吐"的界线；p50 仅 0.11、
# p75 1.36。历史回放（转负+回落≥3+非回马枪）：54 个交易日中位每天命中 1 条、日均 3.98，仅崩盘日
# （2026-06-08）爆量 50 条（当日大盘普跌，大部分推荐确实失败，移除合理）。
# 教训（2026-08-13 两次修正）：① 仅按回落阈值会把推荐时 +8~12% 强势股正常回吐（+11%→+7%）
# 成批误杀（曾"过滤掉一半"）——必须同时要求已转负；② 阈值不可为凑单票（行云科技）而设——
# 行云科技（+3.86%→-3.12%，回落 6.98）在任何 ≥2 的合理阈值下都会被捕获，真正决定作用的是
# 分布尾部（p90）。回马枪为掉榜跟踪池（推荐时刻涨幅=企稳点），不参与自动移出。
# 仅作用于展示层，backtest/nextday_attribution 读 recommendations 不过滤。
REVERSAL_DROP_THRESHOLD = 3.0
