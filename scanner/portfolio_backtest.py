...
def _load_signals(
    conn: sqlite3.Connection, cfg: PBConfig, calendar: list[str], cal_index: dict[str, int], cal_end: str
) -> list[Signal]:
    """从 recommendations 加载信号，并计算买入日 / 卖出日在日历中的下标。

    若 cfg.deheat=True（默认），从 score_breakdown(JSON) + source 列重建「去热度分」：
        deheated = raw_score - Σ(热度组件)
...
def run_backtest(conn: sqlite3.Connection, cfg: PBConfig) -> BacktestResult:
    ...
    min_date, max_date = conn.execute("SELECT MIN(date), MAX(date) FROM daily_kline").fetchone()
...
def _compute_metrics(
    nav: list[tuple[str, float]], trades: list[Trade], cfg: PBConfig, cash_series: list[float]
) -> dict:
    if len(nav) < 2:
...
def print_report(result: BacktestResult) -> None:
    ...
    print("=" * 72)
    print(f"回测窗口(推荐日): start={cfg.start or '全期'}  end={cfg.end or '全期'}  days={cfg.days or '全期'}")
    print(
        f"参数: hold_days={cfg.hold_days}  buy_delay={cfg.buy_delay}  "
        f"buy_at={cfg.buy_at}  "
        f"max_positions={cfg.max_positions}  initial={cfg.initial_capital:,.0f}"
    )
    print(
        f"成本: 佣金={cfg.commission * 10000:.2f}‱(最低{cfg.min_commission:.0f}元)  "
        f"印花税={cfg.stamp_duty * 100:.3f}%  滑点={cfg.slippage * 100:.2f}%(双边)"
    )
    print(f"信号数={result.n_signals}  成交笔数={m.get('n_trades', 0)}  跳过(缺开盘)={result.skipped_no_open}")
    print("-" * 72)
    if "error" in m:
        print(f"  [!] {m['error']}")
        return
...
def print_comparison(results: list[BacktestResult]) -> None:
    ...
    print("=" * 92)
    header = f"{'策略':<18}{'总收益':>9}{'年化':>9}{'Sharpe':>8}{'最大回撤':>10}{'胜率':>8}{'笔数':>7}"
    print(header)
    print("-" * 92)
    for r in results:
        m = r.metrics
        if "error" in m:
            ...
            continue
        print(
            f"{r.label:<18}{m['total_return'] * 100:>8.2f}%{m['annualized'] * 100:>8.2f}%"
...
def main() -> None:
    ...
    parser.add_argument("--compare", action="store_true", help="多策略对比：各现役类别 + 综合 + 基准")
    parser.add_argument(
        "--rescore",
        action="store_true",
    ...
    if args.buy_delay == 0 and args.buy_at == "open":
        ...
        parser.error(
            "--buy-delay 0 + --buy-at open 构成前视偏差：信号收盘后才产生，"
