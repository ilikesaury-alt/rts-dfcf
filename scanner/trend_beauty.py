"""走势美感判定（2026-09-09）：日线 / 分时走势「漂亮」判定。

用户需求（原始场景）：进入终选参考（scanner/final_pick）的票，分时走势与日线走势都要
「漂亮」。纯展示/筛选层：不改评分/排序/落库；数据缺失 fail-open（不判否），
只拦「可判定的丑」，不因数据缺口误杀。
⚠ 2026-09-21：终选参考区整体删除，本模块**不再服务任何准入硬门**（那个门 2026-09-09
数据裁决后本就默认关、随该区一并删除）。现存消费者只有两处，都是展示/独立区：
  · 走势标记 beauty_mark → v1 池选行行尾（经 view.model._beauty_mark_for）；
  · 沪深飙升独立区 B 段（榜外异动）与 hot_watch 的美感门共用 ma_bullish / 本模块判定。
