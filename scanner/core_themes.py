...
本模块补的是「大跌市中主线方向回调低吸」盲区：仍在走强的市场主线性概念（近 N 日
持续上榜/强势）里的核心股，在上涨途中健康回调（非超跌、非破位）的低吸窗口。
（原与「回马枪（掉榜超跌票反弹企稳）」并列为两个弱市盲区补丁；回马枪桶已于
2026-09-16 删除。）
...
复盘验证，但不进综合排序主表 / 回测口径（in_main_table=False），
