...
cur = conn.execute(
    "SELECT DISTINCT r.symbol, r.name FROM recommendations r WHERE r.date = ? ORDER BY r.symbol", (target_date,)
)
stocks = cur.fetchall()
print(f"{target_date} 上榜个股: {len(stocks)} 只\n")

cur = conn.execute(
    """SELECT r.symbol, r.name, r.category, MAX(r.score), r.score_breakdown, r.percent
FROM recommendations r WHERE r.date = ? GROUP BY r.symbol, r.category ORDER BY MAX(r.score) DESC""",
    (target_date,),
)
rows = cur.fetchall()
print("=== 各股最高分（按评分降序）===")
...
cur = conn.execute(
    """SELECT r.symbol, r.name, MAX(r.score), r.percent
...
cur = conn.execute(
    "SELECT r.time, COUNT(*) FROM recommendations r WHERE r.date = ? GROUP BY r.time ORDER BY r.time", (target_date,)
)
cycles = cur.fetchall()
print("=== 扫描轮次 ===")
for c in cycles:
